...
namespace DojoTemplateTestProject.CrossedWiresTests
{
    public class IntersectionFinderTests
    {
        ...
        public void Part2TestEndToEnd(string wireOneInput, string wireTwoInput, int shortestDistance)
        {
            var intersectionFinder = new IntersectionFinder();
            var segmentFactory = new SegmentFactory(intersectionFinder);
            intersectionFinder.WireOneDirections = wireOneInput.Split(",").ToList();
            intersectionFinder.WireTwoDirections = wireTwoInput.Split(",").ToList();

            segmentFactory.LoadSegments();
            intersectionFinder.FindAllIntersections();
            intersectionFinder.CalculateStepsToAllIntersections();
            var result = intersectionFinder.FindShortestDistanceToIntersection();
